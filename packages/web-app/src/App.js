import React, { Component } from 'react';
import { createBrowserHistory } from 'history';
import logo from './logo.png';
import demo_dark from './assets/demo_dark.gif';
import demo_light from './assets/demo_light.gif';
import mac_logo from './assets/mac_logo.svg';
import ReactGA from 'react-ga';
import './App.css';
import Footer from './components/Footer';

ReactGA.initialize('UA-207021914-1');

const history = createBrowserHistory();
history.listen(() => {
  ReactGA.set({ page: window.location.pathname });
  ReactGA.pageview(window.location.pathname);
});

class App extends Component {
  constructor(props) {
    super(props);
    this.state = {
      theme: demo_dark,
    };
  }

  componentDidMount() {
    ReactGA.pageview(window.location.pathname + window.location.search);
  }

  componentDidUpdate() {
    ReactGA.pageview(window.location.pathname + window.location.search);
  }

  toggleTheme() {
    this.setState({
      theme: this.state.theme === demo_dark ? demo_light : demo_dark,
    });
  }

  onClickDownload() {
    ReactGA.event({
      category: 'Downloads',
      action: 'Download for macOS',
    });
  }

  onClickNavBtn(type) {
    if (type) {
      ReactGA.event({
        category: 'Nav click',
        action: 'Report an issue',
      });
    } else {
      ReactGA.event({
        category: 'Nav click',
        action: 'Visit portfolio',
      });
    }
  }

  render() {
    return (
      <div className='App'>
        <div className='App-bg'></div>
        <header className='App-header'>
          <img
            src={logo}
            width='40px'
            height='40px'
            className='App-logo'
            alt='logo'
          />
          <h1>Spotifyrics</h1>
          <ul className='App-nav'>
            <li>
              <span>The app</span>
            </li>
            <li>
              <a
                href='mailto:contact@dsgdsr.me'
                onClick={() => this.onClickNavBtn(0)}
              >
                Report issue
              </a>
            </li>
            <li>
              <a href='https://dsgdsr.me' onClick={() => this.onClickNavBtn(1)}>
                The developer ;)
              </a>
            </li>
          </ul>
        </header>
        <div className='App-container'>
          <div className='App-description'>
            <div className='App-description-container'>
              <h2>Get the lyrics for any track on Spotify</h2>
              <p>
                You can access all lyrics without account login, just need to
                run the Spotify app while using it. The app will run minimized
                in the menu bar, allowing you to access the lyrics at any
                moment. Multiple options will let you configure the app
                interface and preferences as desired, accessing them by clicking
                the tray icon in the menu bar.
                <br></br>
                <br></br>
                The app is only available for macOS, new features and Windows
                support will be developed soon.
                <br></br>
                <br></br>
                <strong>
                  * Project abandoned as Spotify already implemented lyrics
                  natively.
                </strong>
              </p>
              <div className='App-buttons'>
                <a
                  className='mac-download'
                  href='https://spotifyrics.dsgdsr.me/downloads/Spotifyrics-1.0.1.dmg'
                  onClick={() => this.onClickDownload()}
                  download
                >
                  <img
                    src={mac_logo}
                    alt='mac-logo'
                    width='16px'
                    height='16px'
                  />
                  Mac download
                </a>
                <a
                  className='paypal-donate'
                  target='_blank'
                  rel='noreferrer'
                  href='https://www.paypal.com/donate?hosted_button_id=YFXTYPKAALV2E'
                >
                  <img
                    src='data:image/svg+xml;base64,'
                    alt='donate-paypal'
                    width='62px'
                    height='20px'
                  />{' '}
                  Donate
                </a>
              </div>
            </div>
          </div>
          <div className='App-demo'>
            <div className='demo-wrapper'>
              <div className='toggle-wrapper'>
                <span>Toggle app theme:</span>
                <div className='toggle transparent'>
                  <input
                    onChange={(event) => this.toggleTheme()}
                    id='transparent'
                    type='checkbox'
                  />
                  <label className='toggle-item' htmlFor='transparent'></label>
                </div>
              </div>

              <img
                src={demo_light}
                key={demo_light}
                height='0px'
                width='0px'
                alt='logo'
                style={{ overflow: 'hidden', position: 'absolute' }}
              />
              <img
                src={this.state.theme}
                key={this.state.theme}
                height='calc(100% - 80px)'
                width='inherit'
                alt='logo'
              />
            </div>
          </div>
          <Footer />
        </div>
      </div>
    );
  }
}

export default App;
